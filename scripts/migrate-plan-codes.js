/**
 * Manual Plan Migration Script (JavaScript version)
 *
 * Run this to manually migrate all plan codes in the database
 * Usage: node scripts/migrate-plan-codes.js
 */
...
async function main() {
  ...
  for (const setting of allSettings) {
    ...
    if (oldPlan !== newPlan) {
      try {
        ...
        console.error(`❌ ${setting.shop}: Failed to update (${error.message})`);
        errors++;
      }
    } else {
  ...
  if (updated > 0) {
    console.log('\n✨ Migration complete! Visit /app/settings to verify.');
  } else {
